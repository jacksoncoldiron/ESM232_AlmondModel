...
almond_profit_wrapper(Tn = 10.5, P = 50)
str(yearly_anomalies)
...
temp_coeff <- rnorm(mean = 0.0046, sd = 0.1, n = nsamples)
precip_coeff <- rnorm(mean = 0.0043, sd = 0.1, n = nsamples)

coeff_parms <- cbind.data.frame(temp_coeff, precip_coeff)
...
sensitivity_yield <- coeff_sensitivity_df |>
  mutate(
    profit = pmap_dbl(
      list(temp_coeff, precip_coeff, yield_anomaly),
      ~ almond_yield_uncertainty(
